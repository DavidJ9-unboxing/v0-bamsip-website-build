...
// prop of the <VenueHero /> React component. Provide `src`/`out` to override
// the default paths (used for the shared generic hero). When `name` is omitted
// the venue-name line is left off — the frame stays generic.
const VENUES = [
  ...
  { file: "bar-pop.png", name: "Bar Pop" },
  { file: "crazy-pedros-nq.png", name: "Crazy Pedro's NQ" },
  { file: "atlas-bar.png", name: "Atlas Bar" },
  { file: "flawd.png", name: "Flawd" },
  { file: "seven-bro7hers-beerhouse-ancoats.png", name: "Seven Bro7hers Beerhouse Ancoats" },
  { file: "blossom-street-social.png", name: "Blossom Street Social" },
  { file: "northern-monk-refectory-mcr.png", name: "Northern Monk Refectory MCR" },
  { file: "lass-o-gowrie.png", name: "Lass O'Gowrie" },
  { file: "mojo.png", name: "Mojo Manchester" },
  { file: "nest.png", name: "Nest" },
  { file: "stage-and-radio.png", name: "Stage & Radio" },
  { file: "liquor-store.png", name: "The Liquor Store" },
  { file: "port-street.png", name: "Port Street Beer House" },
  { file: "salisbury-ale-house.png", name: "The Salisbury Ale House" },
  { file: "old-pint-pot.png", name: "Old Pint Pot" },
  { file: "ramona.png", name: "Ramona" },
  { file: "the-freemount.png", name: "The Freemount" },
  { file: "courtyard.png", name: "The Courtyard Manchester" },
  { file: "impossible.png", name: "Impossible Manchester" },
  { file: "alberts-schloss.png", name: "Albert's Schloss" },
  { file: "freight-island.png", name: "Escape to Freight Island" },
  // Shared default hero for every venue WITHOUT its own photo. Uses the same
  // new design (no venue name) so all outreach emails are visually consistent.
  {
    src: "public/images/hero-night.png",
    out: "public/images/email-venue-launch-hero.png",
  },
]
...
function frameSvg(name) {
  ...
  return Buffer.from(`<svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- right-hand value block: venue name (optional), big coral "100", strapline -->
...
async function build({ file, name, src: srcOverride, out: outOverride }) {
  const src = srcOverride ?? `${SRC}/${file}`
  const out = outOverride ?? `${OUT}/${file}`
